const landing = new Vue({
    ...
    data: {
        ...
        //Search text using the search button
        searchInput: {
            searchText: "",
        },
    ...
    computed: {
        ...
        filteredMoviesComingSoon() {
            ...
            let orderedMovies = filteredMoviesComingSoon.sort((a, b) => {
                return b.ReleaseDate - a.ReleaseDate;
            })
            return orderedMovies;
        },

        filteredMoviesOnShow() {
            let filteredMoviesOnShow = this.movies.filter((movie) => {
                return movie.Title.toLowerCase().includes(this.searchInput.searchText.toLowerCase());
            })
            return filteredMoviesOnShow;
        },
        ...
        addToWatchList: function (event) {
            ...
            if (this.watchList.length == 0) {
                ...
                for (let i = 0; i < this.watchList.length; i++) {
                    if (this.watchList[i].Title == event.Title) {
                        this.inList = true;
                    } else {
                        this.inList = false;
                    }
                ...
                if (this.inList == true) { }
                    ...
                    else if (this.watchList.length >= 20) {
                        watchList.fullList = true;
                        alert("here too");
                    }
                }
            }
    ...
    mounted() {
        ...
        if (localStorage.getItem('watchList')) this.watchList = JSON.parse(localStorage.getItem('watchList'));
    },
    watch: {
        watchList: {
            handler() {
...
//When the user scrolls down the webpage, the navbar changes the background color
